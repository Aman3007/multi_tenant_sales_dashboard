...
**Hard Isolation at Context Level:**

```javascript
// data/leads.js
export const MOCK_LEADS = {
  'org-a': [/* Organization A leads */],
  'org-b': [/* Organization B leads */]
};

// context/AuthContext.jsx
const tenantLeads = useMemo(() => 
  currentUser ? leads[currentUser.tenantId] || [] : [],
  [currentUser, leads]
);
```
...
1. **Immutable Tenant Association**
   - Each user has a fixed `tenantId`
   - Cannot switch tenants without re-authentication
   - Tenant ID is read-only

2. **Data Segregation**
   - All data objects keyed by `tenantId`
   - Context filters data before passing to components
   - Components never see other tenants' data

3. **Visual Differentiation**
   - Organization A: Blue gradient theme
   - Organization B: Purple gradient theme
   - Clear tenant name in header
   - Color-coded throughout UI

4. **Tenant Context**
   ```javascript
   const tenant = useMemo(() => 
     currentUser ? TENANTS[currentUser.tenantId] : null,
     [currentUser]
   );
   ```
...
// 2. Add tenant data in data/leads.js
export const MOCK_LEADS = {
  'org-c': [/* leads */]
};

// 3. Create users in data/mockUsers.js
export const MOCK_USERS = [
  { id: '5', tenantId: 'org-c', role: 'Admin', ... }
];

// 4. Add color gradient in Header.jsx
